...
void init_football() {
    ...
    Football = create_newwin(court.width, court.height, court.start.x, 
                             court.start.y);//场地
    WINDOW *Message_t = create_newwin(court.width, 7, court.start.x,
                            court.start.y + court.height);//右侧帮助栏
    ...
    Score = create_newwin(20, 7, court.start.x + court.width,
                          court.start.y + court.height);//右下角比分栏
    Write = create_newwin(court.width + 20, 5, court.start.x,
                          court.start.y + court.height + 7);//下方
    return;
}

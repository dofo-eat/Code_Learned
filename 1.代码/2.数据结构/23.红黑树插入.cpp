/*************************************************************************
	> File Name: 23.红黑树插入.cpp
	> Author: dofo-eat
	> Mail:2354787023@qq.com 
	> Created Time: 2020年05月16日 星期六 15时07分11秒
 ************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
Node *erase_maintain(Node *root) {
    ...
    if(root->rchild->color == BLACK) {
        if(root->rchild->rchild->color != RED) {
            //RL类型
        root->rchild->color = RED;//旋转之前的右子树根结点
        root->rchild = right_rotate(root->rchild);
        ...
        root->color = BLACK;
        root->lchild->color -= 1;//删除一冲黑色
        root = left_rotate(root);
        root->rchild->color = BLACK;
    } else {
        if(root->lchild->lchild->color != RED) {
            //RL类型
        root->lchild->color = RED;//旋转之前的右子树根结点
        root->lchild = left_rotate(root->lchild);
...
Node *__erase(Node *root, int key) {
    ...
    if(root->key > key) {
        ...
        if(root->lchild == NIL || root->rchild == NIL) {
            ...
            free(root);
            return temp;//度为0 huo 1
        } else {
            Node *temp = prcdeccesser(root);
            root->key = temp->key;

...
if [[ $memsum -gt 0 ]];then
    for i in ${mem[*]};do
        eval $(ps -aux -q $i -h | awk -v memcount=0 '{if($3>=50){memcount+=1;printf("mem2["memcount"]=%di;mem1["memcount"]=%.40s;mem3["memcount"]=%s;mem4["memcount"]=%2.2f;mem5["memcount"]=%2.2f;",$2,$11,$1,$3,$4);}} END{printf("memcount=%d;", memcount);}')
    done
fi

if [[ $cpusum -eq 0 && $memsum -eq 0 ]];then
    exit 0
fi
